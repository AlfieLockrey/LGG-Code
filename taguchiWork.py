...
def runFromArray(DP, NF):  
    ...
    return lgg.DoIt(D_c=DP[0], L0_c=DP[1], P0_c=NF[3], gamma_c=NF[0], D_pis=DP[7],
             mu_static_pis = 0 , mu_dynamic_pis = 0,
             P0_pt=DP[3], L0_pt=DP[6], D_pt=DP[7], P_rupt=DP[8], L_b=DP[9],
             D_b=DP[10], P0_b=NF[1], gamma_lg=DP[11], m_pr=DP[4], m_sb=0,
             mu_sb=0, gamma_ic=NF[2], V_ic=DP[5], C = DP[12], BR_exp = NF[4], 
...
""" DESIGN PARAMETERS  for L27"""
"""
D_c = [19.685e-3, 18.52e-3, 16.81e-3]             #0
L0_c = [150e-3, 150e-3, 150e-3]        #1
D_pis = [1, 1, 1]           #2
P0_pt = [1e+5, 1e+6, 1e+7]             #3
m_pr = [1.45e-3, 1.45e-3, 1.45e-3]            #4
V_ic = [0.5, 1, 1.5]                   #5
L0_pt = [1, 1.5, 2]                    #6
...
while workable is False:
    index+= 1
    L27index = SNratios.index(orderedSNratios[index])
    
    # Check if P0_pt < P_rupt and that the values are nan or +/- inf
    if expL27[L27index][3] >= expL27[L27index][8] or np.isnan(orderedSNratios[index]) == True or np.isinf(orderedSNratios[index]) == True:
        workable = False
        print(L27index, " is not workable")
    else:
        workable = True
        print(L27index, " is workable")
